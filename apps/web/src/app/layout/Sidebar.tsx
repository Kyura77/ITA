...
import { Command, MonitorSmartphone, X } from "lucide-react";
...
export function Sidebar({ collapsed, setCollapsed, mobileOpen, setMobileOpen }: SidebarProps) {
  ...
  const content = (
    <>
      <div className="px-3 pt-4">
        <div className="panel-hero p-4">
          <div className="flex items-start justify-between gap-3">
            <div className={cn("transition-all", collapsed ? "hidden" : "block")}>
            ...
            <button type="button" className="btn-ghost hidden lg:inline-flex" onClick={() => setCollapsed(!collapsed)}>
              {collapsed ? ">" : "<"}
            </button>
          </div>
        </div>
      ...
      <nav className="mt-6 flex-1 space-y-1 px-3 pb-4">
        {navItems.map(({ to, label, summary, icon: Icon }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              cn(
                "flex items-start gap-3 rounded-[22px] border px-3 py-3 transition",
                isActive
                  ? "border-cyan-400/25 bg-cyan-400/10 text-cyan-100"
                  : "border-transparent bg-transparent text-slate-300 hover:border-white/5 hover:bg-white/5 hover:text-white",
      ...
      <div className={cn("space-y-3 px-3 pb-4", collapsed && "hidden")}>
  ...
  return (
    <>
      <aside className={cn("fixed inset-y-0 left-0 z-40 hidden border-r border-white/5 bg-slate-950/65 backdrop-blur-2xl transition-all duration-300 lg:flex lg:flex-col", collapsed ? "w-[112px]" : "w-[320px]")}>
        {content}
      </aside>

      <div className={cn("fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-sm lg:hidden", mobileOpen ? "block" : "hidden")} onClick={() => setMobileOpen(false)}>
        <aside className="h-full w-[88vw] max-w-[320px] border-r border-white/10 bg-slate-950/95" onClick={(event) => event.stopPropagation()}>
